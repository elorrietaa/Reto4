...
public class ControladorRegistro implements ActionListener {
     ...
     * @param modelo:     Guarda el objeto modelo para poder acceder a los metodos
     *                    del modelo
     * @param controlador Instancia del controlador para poder acceder a los
     *                    distindos elementos del controlador
     */
	...
	case "Registrarse":
		...
		if (modelo.consultas.buscarClientePorDNI(dni) != null) { // Comprueba si existe un usuario con el mismo
									 // DNI
		    JOptionPane.showMessageDialog(vista, "Ya existe un usuario con ese DNI " + this.dni, "Aviso",
			    JOptionPane.WARNING_MESSAGE); // Si existe, muestra un mensaje de error
		} else if (((fechaActual.getTime() - fecha.getTime()) / 86400000) < 6570) { // no se pueden registrar
											    // menores
		    JOptionPane.showMessageDialog(vista,
			    "Lo sentimos. Debe tener al menos 18 años para poder registrarse. Gracias. ", null, 0);
     ...
     * 
     * @return Retorna false si los datos han dado un fallo en la validacion e inica
     *         cual es el fallo. Retorna true si la validacion ha sido correcta
     */
    public boolean validarDatos() { // Funcion que valida los datos de registro

	String nombre = vista.registro.txtNombre.getText(); // Guarda la informacion de la interfaz en diferentes
							    // variables
	...
	// comprobamos que haya sido seleccionado un sexo y le asignamos el valor
	...
	if (controlador.funcionesRegistro.validarDNI(dni) == false) {
	    JOptionPane.showMessageDialog(vista, "DNI no introducido o el introducido no es valido.", "Aviso",
	...
	if (contrasena2.equals("")) {
	    JOptionPane.showMessageDialog(vista, "Confirmar contraseña no introducida.", "Aviso",
		    JOptionPane.WARNING_MESSAGE);
	    return false;
	}
	...
	if (!contrasena.equals(contrasena2)) {
	    JOptionPane.showMessageDialog(vista, "Las contraseñas no son iguales", "Aviso",
		    JOptionPane.WARNING_MESSAGE);
	    return false;
	} else {
