...
public class FuncionesReserva {
	...
	if (tiposAloj.getCodTipoAlojamiento() == 10) {
	    precioReserva = precioReserva + calcularPrecioPorTarifa(tiposAloj, modelo.habitacion.getPrecioHabitacion()); // en
															 // este
															 // caso
															 // se
															 // coge
    ...
    /**
     * Método calcularPrecioPorTarifa, calcula el precio en función de las fechas
     * seleccionadas aplicando el precio de la tarifa normal o tarifa estival en
     * función del número de noches normales y numero de noches en fechas de tarifa
     * estival seleccionadas
     * 
     * @param precioAloj Variable que almacena el precio del alojamiento
     * @param tiposAloj  Variable que guarda el tipo de alojamiento que se ha
     *                   seleccionado
    ...
    /**
     * Método calcularSuplementoFestivos calcula el precio del alojamiento teniendo
     * en cuenta los suplementos festivos.
     * 
     * @param precioAloj Variable que almacena el precio del alojamiento
    ...
    /**
     * Método mostrarDetTipoTarifaCasApart se muestran en
     * vista.detallesReservaCasaApart los detalles de la tarifa aplicada al precio.
     * 
     * @param NumNochesPorTarifa   Variable que almacena el numero de noches que se
     *                             aplica a cada tarifa
	...
	vista.detallesReservaCasaApart.textDetPrecioFinal
		.setText((String) "El precio final tras aplicar las tarifas para su reserva de " + numNoches
			+ " noches es de: " + (String.format("%.2f", precioTarifaAplicada)) + "€");
    }

    /**
     * Método calcularNumNochesPorTarifa = calcula el número de noches a las que se
     * le aplica la tarifa normal y el número de noches a las que se aplica la
     * tarifa estival
     * 
     * @return NumNochesPorTarifa = es un array que contiene en su primera posición
     *         [0] el número de noches a las que se aplica la tarifa normal y en [1]
	...
	for (int i = 0; i < modelo.fechasTarifas.diasFestivos.length; i++) { // se recorren todos los festivos
		...
		nombreFestivosReser = nombreFestivosReser + " Día: " + diasFestivos[i] + " - " + " Mes: "
			+ (mesesFestivos[i] + 1) + " - " + " Festivo: " + nombreFestivos[i]; // se añade el nombre del
											     // festivo a
											     // nombreFestivosReser
	    }
	}
	...
	// los contempla.
	if (diaIda == 1 && mesIda == 0 || (mesIda == 11 && mesVuelta == 0)) { // FESTIVO AÑO NUEVO 1-01 .El mes de Enero
									      // (posición 0) es especial por ser el
									      // anterior el mes dic(posicion11)
		...
		&& (diaVuelta >= 6 && mesVuelta == 0 || mesVuelta == 1)) { // FESTIVO REYES 6-01 .El mes de Enero
									   // (posición 0) es especial por ser el
									   // anterior el mes dic(posicion11)
	    numNochesFestivos = numNochesFestivos + 1;
		...
		&& (diaVuelta >= 8 && mesVuelta == 11 || mesVuelta == 0)) { // FESTIVO DIA DE LA CONCEPCION 6-11 .El mes
									    // de diciembre (posición 11) es especial
									    // por ser el posterior al mes
									    // enero(posicion0)
	    numNochesFestivos = numNochesFestivos + 1;
		...
		&& (diaVuelta >= 25 && mesVuelta == 11 || mesVuelta == 0)) { // FESTIVO NAVIDAD 25-11 .El mes de
									     // diciembre (posición 11) es especial por
									     // ser el posterior al mes enero(posicion0)
	    numNochesFestivos = numNochesFestivos + 1;
	    nombreFestivosReser = nombreFestivosReser + " Día: " + 25 + " - " + " Mes: " + 12 + " - " + " Festivo: "
		    + "Navidad";
	}
	// FALTA se mete el nombreFestivosReser en el modelo.reserva o se muestra en un
	// jtext
    ...
    /**
     * Método: guardarReservaAlojamiento = Genera la reserva de la casa/alojamientoy
     * la guarda en modelo.reserva
     * 
     ...
     * 
     * @param tiposAloj Variable que guarda un valor numerico referente al tipo de
     *                  alojamiento
     */
	...
	vista.detallesReservaCasaApart.textPDatosAlo
		.setText((String) "Ciudad: " + modelo.reserva.getAlojamiento().getUbicacion()
	...
	// muestra los detalles de las habitaciones del alojamiento: cuales tiene y
	// cuantas
     ...
     * 
     * @param listaCamas Array list con la lista de camas de la habitación
     *                   seleccionada
     * @return tiposCamaHab
     */
    ...
    /**
     * El método guardarNumHabTipoDeCasaApart rellena un array arrayNumHabitaciones
     * que contiene el número de habitaciones de cada tipo de habitacion y lo guarda
     * en el modelo
     * 
     ...
     * 
     * @param listaCamas Array list que almacena la lista de camas para la
