...
public class ControladorPanPago implements ActionListener {
    ...
    public float[] monedas; // Almacena el valor de las monedas y billetes que se van introducioendo para
			    // luego poder retarlos.
     ...
     * 
     * @param vista:  Guarda el objeto vista para poder utilizar los distintos
     *                elementos de la interfaz
     * @param modelo: Guarda el objeto modelo para poder acceder a los metodos del
     *                modelo
     */
	...
	String botonPulsado = ((JButton) e.getSource()).getActionCommand(); // Almacena el texto del boton para poder
									    // identificarlo posteriormente
	float importe; // Almacena el importe que representa cada uno de los botones, para así poder
		       // realizar las operaciones con el dinero
	...
	case "Continuar":
		...
		JOptionPane.showMessageDialog(vista,
			"Lo sentimos. No puede continuar su compra hasta que abone el importe completo. Gracias. ",
			null, 0);
    ...
    /**
     * Realiza las operaciones con el importe de cada boton, tambien almacena las
     * cantidades que se ban introduciendo, se calcula el dinero que fala por
     * introducir o lo que sobra
     * 
     * @param importe: Variable que guarda el valor monetario de cada uno de los
     *                 botones de pago, para luego pasarlo como parametro en la
     *                 funcion hace los calculos con el dinero
     */
    public void FuncionBotonDinero(float importe) {
	int posicion; // Guarda la posicion en la que se debe almacenar el importe del boton pulsado
		      // en al array de monedas
	...
	monedas = FuncionesPago.redimensionarArrayMayor(monedas); // Redimensiona el array monedas a un array una
								  // posicion mayos para poder guardar el importe del
								  // boton pulsado
	posicion = monedas.length - 1;
	monedas[posicion] = importe; // Guarda el importe en la ultima posicion del array
	introducido.setText(Float.toString(dinero) + " €"); // Muestra en la interfaz el dinero que se ha introducido
							    // hasta el momento
	if (dinero < total) { // Comprueba si todabia falta dinero para llegar al total
	    falta = controlador.funcionesPago.falta(total, dinero); // En caso de no llegar o sobrepasar el total
								    // necesario, se calcula el dinero faltante
	    restante.setText(Float.toString(falta) + " €"); // Muestra en la interfaz el dinero que falta para alcanzar
							    // el total
	} else
    ...
    /**
     * Realiza las operaciones necesarias cuando el usuario pide que se le devuelva
     * la ultima moneda introducida
     */
    /*
     * public void FuncionDevolver() { int posicion; if(dinero > 0) { posicion =
     * monedas.length - 1; moneda = monedas[posicion]; // Guarda el valor de la
     * ultima moneda introducida en el array dinero = dinero - moneda; // Resta al
     * dinero total el importe de la ultima moneda introducida dinero =
     * Math.round(dinero*100); dinero = dinero/100;
     * introducido.setText(Float.toString(dinero) + " €"); // Actualiza el indicador
     * del dinero introducido en la interfaz, con el valor que tiene despues de
     * sacar la ultima moneda introducida monedas =
     * modelo.funcionesPago.redimensionarArrayMenor(monedas); if(dinero < total) {
     * // Comprueba si despues de sacar la moneda el dinero introducido hasta el
     * momento es menor que el total falta = modelo.funcionesPago.falta(total,
     * dinero); // Calcula el dinero que falta para llegar al total
     * restante.setText(Float.toString(falta) + " €"); // Muestra en la interfaz el
     * dinero que falta this.vista.pago.btn1cen.setEnabled(true); // Vuelve a
     * habilitar los botones con el dinero this.vista.pago.btn2cen.setEnabled(true);
     ...
     * this.vista.pago.btn1.setEnabled(true); this.vista.pago.btn2.setEnabled(true);
     ...
     * 
     * this.vista.pago.btnContinuar.setVisible(false); // Desabilita el boton de
     * continuar y lo hace invisible this.vista.pago.btnContinuar.setEnabled(false);
     * } else TodoIntroducido(); // Si el dinero es igual o mayor que el total,
     * llama a esta funcion } }
     */
    /**
     * Funcion que se utiliza una vez que el dinero ha llegado o sobrepasado el
     * total
     */
	...
	if (modelo.reserva.getAlojamiento() instanceof Hotel) {
	    ...
	    modelo.consultas.insertar1Reserva(modelo.reserva, modelo.habitacion, modelo.servicios,
		    modelo.cliente.getDni(), modelo.fechaIda, modelo.fechaVuelta, (java.sql.Date) fechaActual,
		    horaActual, modelo.precioTotal);
	...
	// PARA CASAS Y APARTAMENTOS:
		...
		|| modelo.reserva.getAlojamiento() instanceof Apartamento) {
	    modelo.consultas.insertarReservaCasaApart(modelo.reserva, modelo.servicios, modelo.cliente.getDni(),
		    modelo.fechaIda, modelo.fechaVuelta, (java.sql.Date) fechaActual, horaActual, modelo.precioTotal);
	    modelo.consultas.insertarPersonasAlojadas(modelo.listaPersonasEncriptada, modelo.reserva);
	}

	// (3º)Genera un fichero con datos de la reserva, 1 fichero por cada reserva de
	// cada habitación
	// para HOTELES
