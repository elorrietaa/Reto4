...
public class FuncionesFicheroReserva {
    ...
    /**
     * Método: imprimirReservaHabitacionesHotel, escribe en el fichero la reserva de
     * las habitaciones del hotel. 1 reserva por habitacion En el caso de que el
     * fichero que se desea guardar no exista, mustra un mensaje por pantalla y
	...
	try {
		...
		JOptionPane.showMessageDialog(vista,
			"Alerta OJO: ¡¡No es posible guardar la reserva! ¡No existe el archivo en el que quiere guardar la reserva!! LA APLICACIÓN SE CERRARÁ",
			null, 0);
		System.exit(0);
	    }
	    fichero = new FileWriter("C:\\workspace\\RETO4/reserva.txt", true); // true es para añadir al final de un
	    ...
	    writer.println("Nombre: " + modelo.cliente.getNombre() + " Apellidos:" + modelo.cliente.getApellidos()
		    + " DNI: " + modelo.cliente.getDni());
	    writer.println();
	    ...
	    writer.println("Número de camas de la habitación: "
		    + modelo.consultas.buscarNumCamasPorCodHab(modelo.hotel.getCodAlojamiento()));

	    // Mostrar detalles de las camas de la habitación seleccionada:
	    ArrayList<Cama> listaCamas = modelo.consultas
		    .buscarCamaPorCodigoHabitacion(modelo.listaReservas.get(pos).getHabitacion().getCodHabitacion());

	    writer.println("Tipo de camas de la habitación: " + listaCamas.toString());
	    writer.println();
	    writer.println("Precio habitación: "
		    + (String.format("%.2f", modelo.listaReservas.get(pos).getHabitacion().getPrecioHabitacion())
			    + "€/ 1 noche"));
	    writer.println(
		    "Precio habitación: " + (String.format("%.2f", modelo.listaReservas.get(pos).getPrecioReserva())
			    + "€/ " + modelo.numNoches + " noches"));
	    writer.println();
	    writer.println("*************************************************************************************");
	    writer.println();
	    writer.println("PRECIO FINAL TOTAL de todas sus habitaciones reservadas para " + modelo.numNoches
		    + " noches: " + modelo.precioTotal + " €");
	    writer.println();
	    writer.println("*************************************************************************************");
	    writer.flush();
	} catch (Exception e) {
    ...
    /**
     * Método: imprimirReserva1HabHotel, escribe en el fichero la reserva de las
     * habitaciones del hotel. 1 reserva por habitacion En el caso de que el fichero
     * que se desea guardar no exista, mustra un mensaje por pantalla y cierra la
     * aplicación.
     * 
     * @param modelo
     * @param vista
	...
	try {
	    ...
	    writer.println("Número de camas de la habitación: "
		    + modelo.consultas.buscarNumCamasPorCodHab(modelo.reserva.getHabitacion().getCodHabitacion()));

	    // Mostrar detalles de las camas de la habitación seleccionada:
	    ArrayList<Cama> listaCamas = modelo.consultas
		    .buscarCamaPorCodigoHabitacion(modelo.reserva.getHabitacion().getCodHabitacion());
		...
		writer.println("Descuentos aplicados:" + modelo.cuponSeleccionado.getCod_promocional() + " - Descuento:"
			+ (String.format("%.0f", (modelo.cuponSeleccionado.getDescuento() * 100))) + "%");
    ...
    /**
     * Método: imprimirReservaFicheroCasa, escribe en el fichero la reserva En el
     * caso de que el fichero que se desea guardar no exista, mustra un mensaje por
     * pantalla y cierra la aplicación.
     * 
     * @param modelo
     * @param vista
     */
	...
	try {
	    ...
	    writer.println();
	    writer.println("=====DATOS DEL CLIENTE: ===== ");
	    writer.println("Nombre1111: " + modelo.cliente.getNombre() + " Apellidos:" + modelo.cliente.getApellidos()
		    + " DNI: " + modelo.cliente.getDni());
	    writer.println();
	    writer.println("=====DATOS DEL ALOJAMIENTO: ===== ");
	    writer.println("Ciudad: " + modelo.reserva.getAlojamiento().getUbicacion());
	    ...
	    writer.println("Dirección: " + modelo.apartamento.getDireccion() + " Piso: " + modelo.apartamento.getPiso()
		    + "   Teléfono: " + modelo.apartamento.getTelefono());
