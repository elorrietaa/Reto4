...
/**
 * Esta clase controla el panel de detalles y se encarga de enlazar esa parte de
...
public class ControladorPanSelHabitacion implements ActionListener {
     ...
     */
    public ControladorPanSelHabitacion(JframePrincipal vista, PrincipalModelo modelo, Conexion conexion,
    ...
    /**
     * Método: guardar1DormSeleccionadosJTable = se gudardan en el ArrayList
     * listaHabSeleccionadas los datos de las habitaciones seleccionadas.
     */
    public void mostrar1DormSeleccionadosJTable() {
	// indHabsSel es un array que contiene el índice (la posición) de las
	// habitaciones seleccionadas en el JTable
	indHabsSel = vista.selHabitacion.tab.getSelectedRows();

	// la lista de dormitorios DISPONIBLES que semuestra al usuario para que
	// seleccione
	listaDormitorios = consultas.buscarHabitacionDisponiblel(modelo.fechaIda, modelo.fechaVuelta,
		modelo.hotel.getCodAlojamiento());
	...
	// rellenamos la habitacion con los datos de la habitacion seleccionada
	// (listaDormitorios.get(filaDormSel))
	...
	// Se muestran los detalles de las habitaciones seleccionadas en el JTable del
	// panel detallesReserva
	mostratDetallesHabHotelSel(habitacion);

    }

    /**
     * Método mostratDetallesHabHotelSel = Se muestran los detalles de las
     * habitaciones seleccionadas en el JTable del panel detallesReserva
     * 
     * @param habitacion Objeto dormitorion con la informacion de las habitaciones
     *                   del hotel seleccionado
     */
	...
	datos[5] = (String.format("%.2f", modelo.hotel.getPrecioAlojamiento()) + "€ / "
		+ controlador.funcionesReserva.calcularNochesReservadas() + " noches");
	tablaDetCasApart.addRow(datos);

    }

    /**
     * Método: mostrarHoteles muestra los detalles del hotel seleccionado por el
     * usuario en la pantalla PandetallesReserva
     */
	...
	for (int i = 0; i < listaDormSeleccionados.size(); i++) {
	    ...
	    ArrayList<Cama> listaCamas = modelo.consultas
		    .buscarCamaPorCodigoHabitacion(listaDormSeleccionados.get(i).getCodHabitacion());
	    String tiposCamaHab = controlador.funcionesReserva.mostrarTiposDeCamas(listaCamas);
	    datos[3] = tiposCamaHab;
	    datos[4] = (String.format("%.2f", listaDormSeleccionados.get(i).getPrecioHabitacion()) + "€");
	    datos[5] = (String.format("%.2f", modelo.listaReservas.get(i).getPrecioReserva()) + "€ / "
    ...
    /**
     * Método: guardarReservasHab = Genera 1 o varias reservas y las guardar en en
     * el ArrayList<Reserva> listaReservas
     */
    /*
     ...
     * 
     * //creamos un arrayList listaHabSeleccionadas que va a contener las
     ...
     * 
     * //metemos las reservas de las habitaciones seleccionadas en un arrayList
     * listaReservas modelo.reserva = new Reserva(); codReserva = codReserva + 1;
     ...
     * modelo.reserva.setPrecioReserva(controlador.funcionesReserva.
     * calcularPrecioHabXNoches(listaDormSeleccionados, i));
     * listaReservas.add(modelo.reserva); }
     * 
     * //probamos que listaHabSeleccionadas se haya creado y rellenado
     * correctamente: for(int i=0; listaReservas.size()>i; i++) {
     * System.out.println("------->La lista de reservaaaas: " +"Código reserva: "
     * +listaReservas.get(i).getCodReserva() + "cod alojamiento reservado " +
     * listaReservas.get(i).getAlojamiento().getCodAlojamiento() +
     * "codigo habitacion reservada: "+
     * listaReservas.get(i).getHabitacion().getCodHabitacion()
     * +"precio reserva 1 hab: "+ listaReservas.get(i).getPrecioReserva()); }
     * 
     * //guardamos el ArrayList listaReservas en el modelo modelo.listaReservas =
     * this.listaReservas; }
     */
    /**
	...
	case "Continuar":
		 ...
		 * guardarDormSeleccionadosJTable(); guardar1DormSeleccionadosJTable();
		 * 
		 * //(2º)Genera 1 o varias reservas y las guardar en en el ArrayList<Reserva>
		 * listaReservas guardarReservasHab();
		 * 
		 ...
		 * 
		 * //(4º) se actualiza la información del siguiente panel: PanDetallesReserva
		 * con la info del hotel seleccionado, las habitaciones seleccionadas y el
		 * precio Total de la reserva actualizarPanDetallesReserva();
		 * 
		 * //(5º) actualiza el siguiente panel: Desaparece Panel de Seleccionar
		 * habitacion y aparece panel de detalles reserva
		 * vista.selHabitacion.setVisible(false);
		...
		// pasar por parámetro el tiposAloj para diferenciar entre hotel, casa y
		// apartamento ¿? no se si es lo mas correcto
		...
		// (4º) se actualiza la información del siguiente panel: PanDetallesReserva con
		// la info del hotel seleccionado, las habitaciones seleccionadas y el precio
		// Total de la reserva
		controlador.controladorPanBuscarAlojamiento.actualizarPanDetallesReservaCasaAloj();

		// (5º) actualiza el siguiente panel: Desaparece Panel de Seleccionar habitacion
		// y aparece panel de detalles reserva
		vista.selHabitacion.setVisible(false);
		...
		JOptionPane.showMessageDialog(vista,
			"Por favor, seleccione al menos una habitación para continuar. Gracias. ", null, 0);
	    }
	    break;
