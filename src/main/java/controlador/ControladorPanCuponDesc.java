...
public class ControladorPanCuponDesc implements ActionListener {
    ...
    /**
     * Acción de los distintos listeners Si el usuario le da al boton aplicar y
     * continuar , al precio del alojamiento se le aplica el cupon seleccionado y
     * pasa a la pantalla de las bases legales.
     */
	...
	if (sourceObject instanceof JButton) {
	    ...
	    String botonPulsado = ((JButton) e.getSource()).getActionCommand(); // Almacena el texto del boton para
										// poder identificarlo posteriormente
		...
		CodigoPromocional cuponAlojSeleccionado = (CodigoPromocional) vista.cupon.cBListaCupones
			.getSelectedItem();

		// Si ha seleccionado algún código promocional
		if (cuponAlojSeleccionado != null) {
		    // el precioTotalSinCupon será el que hasta ahora estaba calculado
		    // (modelo.precioTotal)
	    ...
	    actualizarJComboBox(); // se actualiza el JTable del PanBuscarHotel con los datos seleccionados por el
				   // usuario
    ...
    /**
     * Método actualizarFiltradoJComboBox = se actualizan los datos del
     * cBlistaCupones Este método actualiza los jcombobox segun lo que seleccionado.
     */
	...
	if (cuponAloj != null) {
	    ...
	    System.out.println("EL descuento seleccionado es: " + modelo.cuponSeleccionado.getCod_promocional()
		    + " del alojamiento " + modelo.cuponSeleccionado.getCod_alojamiento());
	    ...
	    // IMPORTANTE!!! PARA EVITAR ERRORES!! se vuelve a calcular el precio total de
	    // la reserva si el usuario retrocede hacia atrás
	    // muestra el PRECIO TOTAL DE LA RESERVA:
	    controlador.funcionesReserva
		    .calcularPrecioTotalFinalReserva(controlador.controladorPanBuscarAlojamiento.tiposAloj);
	    // se actualiza el precioFinal con descuento aplicado:
	    vista.cupon.tFPrecioSinDesc.setText(Float.toString(modelo.precioTotal) + " €"); // Muesta el precio total
