...
public class ControladorPanDetallesReserva implements ActionListener {
    ...
    /**
     * Método funcionContinuar = contiene los métodos que se ejecutarán cuando el
     * usuario pulse al botón continuar
     */
    public void funcionContinuar() {
	ControladorLogin.panelOrigen = vista.detallesReserva;
	ControladorLogin.detalles = true;

	// Si no se ha iniciado sesión, es decir, si el cliente es null, cuando se pulse
	// el botón continuar se irá al panel PanDetallesReservaAloj
	if (modelo.cliente == null) {
	    vista.login.setVisible(true);
	    vista.detallesReserva.setVisible(false);
	}
	//// Si no se han aceptado las bases, es decir, si el basesAceptadas es null,
	//// cuando se pulse el botón continuar se irá al panel PanBasesLegales
