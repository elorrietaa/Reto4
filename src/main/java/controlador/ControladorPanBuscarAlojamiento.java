...
/**
 * Clase: ControladorPanBuscarHotel: contiene los métodos utilizados en el Panel
 * PanBuscarHotel.
 * 
 * @author IN1DM3B_09
 *
...
public class ControladorPanBuscarAlojamiento implements ActionListener, PropertyChangeListener {
     ...
     */
    public ControladorPanBuscarAlojamiento(JframePrincipal vista, PrincipalModelo modelo, Conexion conexion,
    ...
    /**
     * Método: mostrarCiudad = muestra las ciudades que se han buscado en el método
     * BuscarCiudad (en la BBDD)
     */
    ...
    /**
     * Método: mostrarTiposAloj = muestra las ciudades que se han buscado en el
     * método BuscarCiudad (en la BBDD)
     */
    ...
    /**
     * Método: mostrarOpcionesParaOrdenar = muestra las opciones en base a las que
     * se puede ordenar la búsqueda
     */
    ...
    /**
     * Método: mostrarOpcionesParaOrdenar = muestra las ciudades que se han buscado
     * en el método BuscarCiudad (en la BBDD)
     */
    ...
    /**
     * Método: guardarDatosSeleccionados = guarda los datos seleccionados por el
     * usuario en los objetos.
     */
    ...
    /**
     * Método: guardarDatosSeleccionadosTipoAloj = guarda los datos seleccionados
     * por el usuario en los objetos.
     */
	...
	// Pruebas
	System.out.println("***DATOS Tipo Alojamiento***:" + tiposAloj + "***DATOS Tipo Alojamiento***:"
		+ tiposAloj.getCodTipoAlojamiento());

    }

    /**
     * Método guardarDatosSeleccionadosOrdenarPor = guarda los datos seleccionados
     * por el usuario en los objetos.
     */
    ...
    /**
     * Método guardarDatosSeleccionadosAscDesc = guarda los datos seleccionados por
     * el usuario en los objetos.
     */
    ...
    /**
     * Método: mostrarHotelesEnJTable muestra los alojamientos que se han encontrado
     * mediante el método buscarAlojamientoPorCodigoCiudad en base al
     * codCiudadSeleccionado y codTipoAlojSeleccionado por el usuario
     * 
     * @param codCiudadSeleccionada   se pasa el código de la ciudad seleccionada
     *                                por el usuario por parámetro
     * @param codTipoAlojSeleccionado Se le pasa el código del tipo de alojamiento
     *                                seleccionado por el usuario por parámetro
     * @param ordenarPor              Criterio de busqueda indicado por el usuario
     * @param ascDesc                 se le pasa la selección del usuario
     *                                (ascendente o descendente) que será cómo desea
     *                                ordenar la lista de alojamientos
     */
    public void mostrarAlojamientosEnJTable(int codCiudadSeleccionada, int codTipoAlojSeleccionado, String ordenarPor,
	    String ascDesc) {
	...
	if (codTipoAlojSeleccionado == 10) {
	    ...
	    listaHoteles = controlador.funcionesOrdenar.ordenarListaHoteles(codCiudadSeleccionada,
		    codTipoAlojSeleccionado, ordenarPor, ascDesc);

	    // si no hay hoteles que cumplan con los filtros seleccionados, aparece mensaje
	    // en el panel:
	...
	// ***TABLA CASA O APARTAMENTO es .table
	    ...
	    listaCasas = controlador.funcionesOrdenar.ordenarListaCasas(codCiudadSeleccionada, codTipoAlojSeleccionado,
		    ordenarPor, ascDesc);
		...
		System.out.println("DATOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOS POPULARIDAD casa"
			+ listaCasas.get(i).getPopularidad());
	    }
	    ...
	    // llena el arrayList con la lista de casas
	    listaApartamentos = controlador.funcionesOrdenar.ordenarListaApartamentos(codCiudadSeleccionada,
		    codTipoAlojSeleccionado, ordenarPor, ascDesc);
	    // si no hay apartamentos que cumplan con los filtros seleccionados, aparece
	    // mensaje en el panel:
		...
		datos1[2] = "Dirección: " + listaApartamentos.get(i).getDireccion() + "   Piso: "
			+ listaApartamentos.get(i).getPiso();
		tablaCasApart.addRow(datos1);

		System.out.println("DATOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOS POPULARIDAD apartamento"
			+ listaApartamentos.get(i).getPopularidad());
	    }
	}
    }
     ...
     * 
     * Lo primero que hace es llamar de la base de datos y coge las habitaciones por
     * codigo alojamiento y lo guarda en el arraylist listaHabitaciones tabla Tabla
     * que se rellena con la informacion de las habitaciones
     */
    public void mostrarDetallesHabsCasa() {

	// Mostrar los datos de las habitaciones en tabla de la siguiente pantalla:
	// PanSelHabitacion
	...
	if (modelo.casa != null)
	    listaHabitaciones = consultas.buscarHabitacionDormitorioPorCodigoAlojamiento(casa,
		    modelo.casa.getCodAlojamiento());
	else
	    listaHabitaciones = consultas.buscarHabitacionDormitorioPorCodigoAlojamiento(apartamento,
		    modelo.apartamento.getCodAlojamiento());
	for (int i = 0; i < listaHabitaciones.size(); i++) {

	    datos[0] = listaHabitaciones.get(i).getTipoHabitacion();

	    // se calcula el numero de camas que tiene la habitación en función del codigo
	    // habitación:
	    ...
	    ArrayList<Cama> listaCamas = modelo.consultas
		    .buscarCamaPorCodigoHabitacion(listaHabitaciones.get(i).getCodHabitacion());
    ...
    /**
     * Método: guardarDatosSeleccionados = guarda los datos seleccionados por el
     * usuario en los objetos. Este método guarda el tipo de alojamiento que ha
     * seleccionado el usuario (hotel , casa o apartamento).
     */
	...
	if (codTipoAlojSeleccionado == 10) {
	    ...
	    // prueba:
	    System.out.println(
		    "Hotel seleccionado: " + modelo.hotel.getNombre() + "estrellas" + modelo.hotel.getEstrellas());
	}
	...
	if (codTipoAlojSeleccionado == 20) {
	    ...
	    // prueba:
	    System.out.println("Casa seleccionado: " + modelo.casa.getNombre() + "codigo aloj"
		    + modelo.casa.getCodAlojamiento() + "precio casa:" + modelo.casa.getPrecioAlojamiento());
	}
	...
	if (codTipoAlojSeleccionado == 30) {
	    ...
	    // prueba:
	    System.out.println("Apartamento seleccionado: " + modelo.apartamento.getNombre() + "codigo aloj"
		    + modelo.apartamento.getCodAlojamiento() + "piso" + modelo.apartamento.getPiso());
	}
    }

    /**
     * Método: guardarDatosSeleccionadosFechas = guarda las fechas seleccionadas en
     * el JCalendar en el modelo.
     * 
     */
	...
	for (int i = 0; i < listaDormitorios.size(); i++) {
	    ...
	    // Mostrar detalles de las camas de la habitación seleccionada:
	    ArrayList<Cama> listaCamas = modelo.consultas
		    .buscarCamaPorCodigoHabitacion(listaDormitorios.get(i).getCodHabitacion());
	    String tiposCamaHab = controlador.funcionesReserva.mostrarTiposDeCamas(listaCamas);
	...
	if (sourceObject instanceof JButton) {
		...
		// (2º) Control de fechas: no se pueden hacer reservas anteriores a now() , ni
		...
		// se hace la buscqueda de listaAlojamientos en función de los filtros
		// seleccionados
		...
		// muestra en el JTable los alojamientos filtrados con las selecciones elegidas
		// por el usuario
		mostrarAlojamientosEnJTable(codCiudadSeleccionada, codTipoAlojSeleccionado, ordenarPor, ascDesc);

		// Se muestra en el JTable los alojamientos filtrados con las selecciones
	    ...
	    actualizarFiltradoJComboBox(); // se actualiza el JTable del PanBuscarHotel con los datos seleccionados por
					   // el usuario
    ...
    /**
     * Método actualizarFiltradoJComboBox = actualiza la información del Jtable
     * dependiendo de los valores escogidos por el usuario en losJComboBox Además de
     * actualizar la información de los JcomboBox guarda en las variables lo que ha
     * escogido el usuario.
     */
	...
	if (ciudad != null) {
		...
		// (4º) muestra en el JTable los alojamientos filtrados con las selecciones
		// elegidas por el usuario
		mostrarAlojamientosEnJTable(codCiudadSeleccionada, codTipoAlojSeleccionado, ordenarPor, ascDesc);
	    }
    ...
    /**
     * Listener de la fecha Este método guarda la fecha que ha cogido el usuario
     */
    @Override
    ...
    /**
     * Método: actualizarPanDetallesReserva = se actualiza la información de la
     * reserva en el siguiente panel: PanDetallesReserva Aparece en
     * PanDetallesReserva la informaicón del hotel seleccionado, las habitaciones
     * seleccionadas, el número de noches de la reserva y el precio Total de la
     * reserva
     * 
     */
	...
	// EN EL FUTURO. Muestra los datos de la o las habitaciones que tiene el
	// alojamiento seleccinado
	// mostrarDetallesHabsSelec(tablaHabs);

    }

    /**
     * Método: mostrarDatosAlojamientoJTable = muestra los detalles del alojamiento
     * seleccionado por el usuario en la pantalla PandetallesReserva Este método
     * muestra en la PandetallesReserva un JTable con los servicios adicionales y
     * sus precios.
     */
    ...
    /**
     * Método: mostrarDetallesHabsSelec = muestra los detalles de las habitaciones
     * seleccionadas, es decir, muestra el contenido del arrayList
     * listaHabSeleccionadas
     * 
	...
	// Mostrar detalles de las camas de la habitación seleccionada:
	ArrayList<Cama> listaCamas = modelo.consultas
		.buscarCamaPorCodigoHabitacion(modelo.habitacion.getCodHabitacion());
	String tiposCamaHab = controlador.funcionesReserva.mostrarTiposDeCamas(listaCamas);

	datos[3] = tiposCamaHab;
	datos[4] = (String.format("%.2f", modelo.habitacion.getPrecioHabitacion()) + "€/1 noche");
	// datos[5] = (String.format("%.2f", modelo.reserva.getPrecioReserva() + "€ / "+
	// modelo.numNoches+" noches");
...
//*****************MÉTODOS DEL BOTON CONTINUAR:******************************
    ...
    /**
     * Método continuarHotel = contiene los métodos a los que se llama cuando el
     * usuario le da al botón continuar del panelBuscarHotel y ha seleccionado un
     * Hotel. Este metodo guarda en el alojamiento el hotel, la lista de dormitorios
     * y muestra los detalles de las habitaciones.
     */
	...
	if (vista.buscarAlojamiento.tab.getSelectedRow() != -1) {
		...
		// (4º) muestra en el siguiente panel las habitaciones en funcion del hotel
		// seleccionado por el usuario
		listaDormitorios = consultas.buscarHabitacionDisponiblel(fechaIda, fechaVuelta,
			hotel.getCodAlojamiento());
		for (int i = 0; i < listaDormitorios.size(); i++) {
		...
		if (listaDormitorios.size() == 0) {
		    JOptionPane.showMessageDialog(vista,
			    "Lo sentimos, no existen habitaciones disponibles para ese alojamiento en las fechas elegidas. Por favor, realice otra selección. Gracias. ",
			    null, 0);
		    continuar = false;
		}

		// (5º)MOSTRAR HABITACIONES Y CAMAS EN JTABLE: MÉTODO
		// buscarCamaPorCodigoHabitacion EXISTE EN CONSULTAS
		mostrarDetallesHabs();
		...
		JOptionPane.showMessageDialog(vista,
			"Por favor, guarde los servicios seleccionados para el alojamiento actual o cancele su seleccion de servicios. Gracias",
			null, 0);
	    }
	} else
	    JOptionPane.showMessageDialog(vista, "Por favor, seleccione un alojamiento para continuar. Gracias.", null,
		    0);
    }

    /**
     *
     * Método continuarCasaApart = contiene los métodos a los que se llama cuando el
     * usuario le da al botón continuar del panelBuscarHotel y ha seleccionado una
     * Casa o un Apartamento. Valida la disponibilidad de casas y apartamentos. Este
     * metodo guarda en el alojamiento los datos de casas y alojamiento dependiendo
     * de que ha seleccionado el usuario.
     */
	...
	if (vista.buscarAlojamiento.table.getSelectedRow() != -1) {
		...
		// (1º) se compueba la disponibilidad del alojamiento para las fechas
		// seleccionadas.
		...
		// (3º) se muestran en la siguiente pantalla los detalles de la reserva y el
		// precio TOTAL de la reserva
		controlador.funcionesReserva.mostrarDatosReservaAloj(tiposAloj);

		// (4º) se actualiza la información del siguiente panel:
		// PanDetallesReservaCasaApart con la info del alojamiento seleccionado y el
		// precio Total de la reserva
		...
		if (disponible == false) {
		    JOptionPane.showMessageDialog(vista,
			    "Lo sentimos, ese alojamiento no está disponible en las fechas seleccionadas. Por favor, haga otra selección. Gracias. ",
			    null, 0);
		}
		// (5º) actualiza el siguiente panel: Desaparece Panel de buscarHotel y aparece
		// panel de detalles reservaCasaAloj
		// para que esto pase:
		// continuar == true --> fecha introducida por el usuario tiene que ser válida.
		// disponible != false -->alojamiento seleccionado tiene que estar disponible
    ...
    /**
     * Método validarCasaApartDisponible comprueba si la casa o el apartamento
     * seleccionado está disponible para las fechas seleccionadas
     */
    public void validarCasaApartDisponible() {
	// PARA CASAS:
	if (tiposAloj.getCodTipoAlojamiento() == 20) {
	    disponible = modelo.consultas.buscarSiAlojDisponible(modelo.fechaIda, modelo.fechaVuelta,
		    modelo.casa.getCodAlojamiento());
